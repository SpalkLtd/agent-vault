...
- **Pluggable Credential Stores**: Back a vault with an external system like [Infisical](https://infisical.com) instead of the local encrypted store. Set `INFISICAL_URL` and create vaults with `--credential-store=infisical`. Infisical-backed vaults also expose [dynamic secrets](https://infisical.com/docs/documentation/platform/dynamic-secrets/overview) as `<NAME>_<FIELD>` credentials, leased on demand. See [docs/learn/credential-stores](docs/learn/credential-stores.mdx).
